...
import basicSort, os, time, shutil, genHTML, initMASC, deltaStorm, paramiko, re, threading, glob, sys, brighten

PATH =  'C:\\Users\\Cooper\\Desktop\\masc_showcase\\' # make sure ends with \
SERVER = '/uufs/chpc.utah.edu/common/home/snowflake3/showcase/' # "
REFRESH = 30 # how often to check folder and update (seconds)
...
class Start:
    ...
    def connect(self, backoff=0):
        try:
            ...
            self.ssh.connect('kingspeak33.chpc.utah.edu', username=self.name.get(), pkey=paramiko.RSAKey.from_private_key_file(PATH+"id_rsa"))
            print('Connected')
            return True
    ...
    def checkDir(self):
        ...
        for i in range(0,dif):
            ...
            try:
                genHTML.genOutputHTML(PATH+'chosen\\', PATH+'chosen\\', 'index.html', 10, 8, 120, 120, 10000, False, False, 5)
            except Exception as e:
    ...
    def updateUI(self):
        ...
        for i, imagepath in enumerate(self.subset):
            ...
            with Image.open(imagepath[0]) as img:
                ...
                formatted = time.strftime(f"%H:%M sharpness:{round(imagepath[3],1)}, size:{round(imagepath[4], 1)}", time_struct)
    ...
    def getFilters(self):
        ...
        # IMAGE SUBSET
        self.frame2 = LabelFrame(root,text=f'Most recent snowflakes this hour that match criteria',padx=15, pady=15)
